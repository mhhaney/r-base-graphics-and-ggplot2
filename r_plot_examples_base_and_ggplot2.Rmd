...
Another way to color your plot is by using rcolorbrewer. We first have to load rcolorbrewer like so:
```{r}
...
display.brewer.all()
```
to see all the available paletes. To view a single palete, type
`display.brewer.pal(n, name)` where name denotes the palete you want to display, while n denotes how many colors are in that palete. For example, display.brewer.pal(3, RdYlBlu shows you the first three colors in the RdYlBl (red, yellow, blue) pallete. `brewer.pal(n, name)` returns the hex codes of the n colors in the name palete, and can be assigned as a vector. 
```{r}
Bl <- brewer.pal(9, "Blues")
```
From there, go back to your plot's color argument and type something like
```{r}
plot(covidchart$positive, covidchart$death, main = "Covid Infection and Fatalities", xlab = "Infections", ylab = "Fatalities", col = Bl[1])
```
To set the color of the plot to the first color in the "Bl" palette we made with colorbrewer.
...
In order to use colorbrewer in ggplot2, you have to first save your ggplot in a variable, and then append the function scale_color_brewer() to it, with the argument pallete =."palletename" inside the function. Like so:
```{r}
covid <- ggplot(data = covidchart, aes(positive, death)) + geom_point(colour = "red") + labs(x = "Infections", y = "Fatalities", title = "Covid Infection and Fatalities") + geom_smooth(method = lm)
covid + scale_color_brewer(palette = "Greens")
```
## Bubble Plot
...
## HeatMap
My attempt to make it from scratch
```{r}
library(ggplot2)
library(dplyr)
library(reshape)
L <- read.csv('heatmap.csv')

L1 <- 
  L %>%select(country,X2000,X2001,X2002,X2003,X2004,X2005,X2006, X2007, X2008,X2009,X2010,X2011,X2012, X2013, X2014, X2015)

L2 <- 
  L1 %>%filter(country==”Argentina”|country==”Brazil”|country==”Chile”|country==”Colombia”|country==”Portugal”|country==”Barbados”|country==”Mexico”|country==”Spain”|country==”Germany”|country=”Egypt”)
...
plot <-ggplot(L3, aes(variable, country, fill= value)) + 
 geom_tile()

plot1<- plot+xlab(“Year”)+
 ylab(“Country”)+
 ggtitle(“Per Capita Carbon dioxide Emission from 2000-2015 in the 10 Most Populous Countries”)+
