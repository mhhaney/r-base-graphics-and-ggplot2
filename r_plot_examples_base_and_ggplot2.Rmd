...
We then draw a regression line based on positive cases and fatalities.
```{r}
covidchart <- read.csv("data/pennsylvania-history.csv")
plot(covidchart$positive, covidchart$death, main = "Covid Infection and Fatalities", xlab = "Infections", ylab = "Fatalities")
...
### ggplot2
ggplot2 is much the same. we set covidchart as the source of our data and positive and death as our x and y values in aes(). geom_point() is called to make ggplot graph this as a scatterplot. The labs() function labels our x and y axes, and the graph overall. To add a regression line, 
```{r}
ggplot(data = covidchart, aes(positive, death)) + geom_point() + labs(x = "Infections", y = "Fatalities", title = "Covid Infection and Fatalities") + geom_smooth(method = lm)
```                          
Regressio line to be added
## Bubble Plot
