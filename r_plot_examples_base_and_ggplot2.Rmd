...
Here is an example where the ggplot2 plot takes more work than the base R plot. Need to use `Pclass` as a factor or manipulate the x-axis scale to make sure it only shows 1, 2, and 3. Used the `scale_color_brewer()` function to set this plot to the same RColorBrewer palette as the base R example.

```{r}
ggplot(data = titanic, aes(x = Pclass, y = Age, color = as.factor(Pclass))) +
...
#                                height = unit(1, "npc"), just = "centre", hjust = NULL, vjust = NULL, interpolate = TRUE, default.units = 'npc', name = NULL, gp =gpar(), vp= NULL) +
#                       stat_bkde2d(bandwith = c(.00004, .00004), aes(fill=..level..), geom ='polygon',
