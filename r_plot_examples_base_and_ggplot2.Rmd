...
display.brewer.all()
```
to see all the available palletes. To view a single pallete, type
```{r}
display.brewer.pal(n, name)
```
name denotes the pallete you want to display, while n denotes how many colors are in that pallete. For example, display.brewer.pal(3, RdYlBlu shows you the first three colors in the RdYlBl (red, yellow, blue) pallete. 

```{r}
brewer.pal(n, name)
```
returns the hex codes of the n colors in the name pallete, and can be assigned as a vector. 
```{r}
